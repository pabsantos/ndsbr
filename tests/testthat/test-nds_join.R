...
test_that(
  "nds_join_spdlimit() throws missing errors", {
    expect_error(
      nds_join_spdlimit(osm_data = ippuc_road_axis),
      "'ndsbr_data' is missing"
    )
    expect_error(
...
test_that(
  "nds_join_spdlimit() throws invalid class errors", {
    expect_error(
      nds_join_spdlimit(ndsbr_data = "xyz", ippuc_road_axis),
      "'ndsbr_data' is not a 'sf' object"
    )
    expect_error(
    ...
    expect_error(
      nds_join_spdlimit(ndsbr_data_sf, ippuc_road_axis, c(1, 2, 3)),
      "'vars' is not a 'character' object"
    )
  }
)
...
test_that(
  "nds_join_spdlimit() returns invalid 'geometry' errors", {
    expect_error(
      nds_join_spdlimit(
        ndsbr_data = ippuc_road_axis,
        osm_data = ippuc_road_axis
      ),
      "Invalid 'ndsbr_data' geometry type"
    )
    expect_error(
